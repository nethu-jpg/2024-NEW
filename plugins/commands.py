...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton('Pika PI', url=f'https://t.me/infinitylk')
            ]]
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton("Me in ur Chat", url=f"http://t.me/{temp.U_NAME}?startgroup=true")
            ],[
            InlineKeyboardButton("Me in u", switch_inline_query_current_chat='')
            ]]
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            return
        btn = [[InlineKeyboardButton("join Here", url=invite_link.invite_link)]]
